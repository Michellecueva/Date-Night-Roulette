...
//                else {continue}
//
            let fbYelpEvent = FBEvents(title: event.categories.first?.title, address: event.location.displayAddress.first, eventID: event.id, description: event.categories.first?.title.description, imageURL: event.imageURL ?? "Image Unavailable", websiteURL: event.url ?? "URL Unavailable", type: preference)
